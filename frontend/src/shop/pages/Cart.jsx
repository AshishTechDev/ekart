...
const cartReducer = (state, action) => {
  ...
  if (action.type === "UPDATE_CART") {
    ...
    const existingProductIndex = updatedCart.findIndex((p) => {
      return p.productId._id === action.payload.productId;
    });
...
export default function Cart() {
  ...
  useEffect(() => {
    const getCartProduct = async () => {
      ...
      // console.log(responseText.cartItems);
      if (!response.ok) {
        return false;
      }
      dispatchCart({ type: "SET_CART", payload: responseText.cartItems });
      dispatchCart({ type: "CALCULATE_TOTAL" });
    };
    getCartProduct();
  }, []);
  ...
  const productSummary = [
    {
      title: "items",
      value: `Rs.${cartState.totalAmount}`,
    },
    {
      title: "Delivery",
      value: "free",
  ...
  const deleteCartItemHandler = async (prodId) => {
    ...
    const responseText = await response.json();
    dispatchCart({ type: "DELETE_CART_ITEM", payload: prodId });
    dispatchCart({ type: "CALCULATE_TOTAL" });
  };

  const updateCartItemHandler = async (qty, prodId) => {
    const response = await fetch(
      ...
      {cartState.cartItems.length > 0 && (
        <div className={styles["container"]}>
          ...
          <div className={styles["cart-checkout-details"]}>
            ...
            <Link className={styles["Checkout"]} to="/address">
              Checkout
            </Link>
          </div>
        </div>
